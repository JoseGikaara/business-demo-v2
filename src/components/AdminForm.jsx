...
export default function AdminForm({ business, onChange, onPreview, onBulk }) {
  ...
  const updArr = (arr, i, key, val) => {
    const next = [...business[arr]]
    next[i] = { ...next[i], [key]: val }
    onChange({ ...business, [arr]: next })
  }
    ...
    <div style={{ minHeight: '100vh', background: '#020617', color: '#e2e8f0', fontFamily: "'Outfit', sans-serif" }}>
      ...
      <div style={{ background: '#0f172a', borderBottom: '1px solid #1e293b', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button onClick={onPreview} style={{ background: 'linear-gradient(135deg, #0ea5e9, #06b6d4)', border: 'none', borderRadius: 8, padding: '10px 24px', color: '#fff', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>
            👁️ Preview Demo Site
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 24, background: '#0f172a', padding: 8, borderRadius: 10, border: '1px solid #1e293b', flexWrap: 'wrap' }}>
          {tabs.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{
              ...
              fontSize: 12, fontWeight: 600, background: activeTab === tab ? '#0ea5e9' : 'transparent',
              color: activeTab === tab ? '#fff' : '#64748b', transition: 'all 0.2s',
            }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                {activeTab === tab && <span style={tabBadge} />}
                {tabLabels[tab]}
              </span>
            </button>
          ))}
            ...
            <div style={card}>
              ...
              <div style={{ display: 'grid', gap: 16 }}>
                ...
                <div>
                  ...
                  {business.mapSearch && <iframe
                      src={"https://maps.google.com/maps?q=" + encodeURIComponent(business.mapSearch) + "&output=embed"}
                      style={{ width: '100%', height: '200px', borderRadius: '8px', border: 'none', marginTop: '10px' }}
        ...
        {activeTab === 'faq' && (
          <div style={card}>
            <div style={cardTitle}>FAQ — Frequently Asked Questions</div>
            <p style={{ color: '#64748b', fontSize: 13, marginBottom: 16 }}>These appear as an accordion on the demo site. Add questions clients typically ask.</p>
            <div style={{ display: 'grid', gap: 12 }}>
              {(business.faqs || []).map((faq, i) => (
                <div key={i} style={{ background: '#0f172a', borderRadius: 10, padding: 16, border: '1px solid #334155' }}>
                  ...
                  <div style={{ marginBottom: 10 }}><label style={lbl}>Question</label><input style={inp} value={faq.q} onChange={e => updArr('faqs', i, 'q', e.target.value)} placeholder="e.g. Do I need to book in advance?" /></div>
        ...
        {activeTab === 'gallery' && (
          <div>
            <div style={card}>
              ...
              <p style={{ color: '#64748b', fontSize: 13, marginBottom: 16 }}>Paste direct image URLs (from Google Photos, Dropbox, Imgur, etc). These show in the gallery section. Leave empty to use category-matched stock photos.</p>
              <div style={{ display: 'grid', gap: 10 }}>
                {(business.galleryImages || []).map((url, i) => (
                  <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    ...
                    <button onClick={() => { const next = business.galleryImages.filter((_, idx) => idx !== i); u('galleryImages', next) }} style={{ background: '#7f1d1d', border: 'none', borderRadius: 6, padding: '6px 12px', color: '#fca5a5', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>✕</button>
                  </div>
                ))}
            ...
            <div style={card}>
              ...
              <p style={{ color: '#64748b', fontSize: 13, marginBottom: 16 }}>Paste 3–6 direct image URLs from their Facebook/Instagram posts. These show in the Social section with a "View More" button linking to their profile.</p>
              <div style={{ display: 'grid', gap: 10 }}>
                {(business.socialImages || []).map((url, i) => (
                  <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    ...
                    {url && <img src={url} alt="" style={{ width: 48, height: 48, objectFit: 'cover', borderRadius: 6, border: '1px solid #334155' }} onError={e => e.target.style.display='none'} />}
                    <button onClick={() => { const next = business.socialImages.filter((_, idx) => idx !== i); u('socialImages', next) }} style={{ background: '#7f1d1d', border: 'none', borderRadius: 6, padding: '6px 12px', color: '#fca5a5', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>✕</button>
                  </div>
                ))}
              ...
              <div style={{ marginTop: 12, padding: 12, background: '#0f172a', borderRadius: 8, fontSize: 12, color: '#64748b' }}>
                💡 Tip: On Facebook/Instagram, right-click any post image → "Copy image address" and paste it here.
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'design' && (
          <div style={card}>
            ...
            <div style={{ marginTop: 20, padding: 20, borderRadius: 8, background: `linear-gradient(135deg, ${business.primaryColor}, ${business.accentColor})` }}>
              ...
              <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: 13, marginTop: 4 }}>This gradient will be used as the brand accent throughout the site</div>
            </div>
          </div>
        )}
        ...
        {copied && (
          <div style={{ marginTop: 12, padding: '12px 16px', background: '#052e16', border: '1px solid #16a34a', borderRadius: 10 }}>
            <div style={{ fontSize: 12, color: '#4ade80', fontWeight: 600, marginBottom: 4 }}>✅ Link copied — send this via WhatsApp:</div>
